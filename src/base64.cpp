...
std::string base64url_encode(const uint8_t *data, size_t len)
{
    ...
    for (size_t i = 0; i < len; i += 3) {
        ...
        if (i + 1 < len) n |= uint32_t(data[i + 1]) << 8;
        if (i + 2 < len) n |= uint32_t(data[i + 2]);
        out.push_back(url_table[(n >> 18) & 0x3f]);
        out.push_back(url_table[(n >> 12) & 0x3f]);
        if (i + 1 < len) out.push_back(url_table[(n >> 6) & 0x3f]);
        if (i + 2 < len) out.push_back(url_table[n & 0x3f]);
    }
    return out;  // no '=' padding
}

static int decode_url_char(char c)
{
    if (c >= 'A' && c <= 'Z') return c - 'A';
    if (c >= 'a' && c <= 'z') return c - 'a' + 26;
    if (c >= '0' && c <= '9') return c - '0' + 52;
